...
"""
Analye a given npz file following some of the Buckeye naming conventions.
...
def check_argv():
    ...
    parser.add_argument("npz_fn", type=str, help="the npz archive to analye")
    if len(sys.argv) == 1:
        parser.print_help()
        sys.exit(1)
...
def main():
    ...
    for key in keys:
        N, D = npz[key].shape
        lengths.append(N)
    print("Minimum length (frames):", min(lengths))
    ...
    print(
        "Variance vector for", filter_terms, "entries:", data[:,:].var(axis=0)
        )
