...
# Herman Kamper, kamperh@gmail.com, 2014-2015, 2018.
...
[ ! -f $utterance_ids ] && ./get_npz_keys.py $features_npz $utterance_ids
[ ! -f $labels ] && ./create_labels.py $utterance_ids $labels

# Generate a list of all possible pairs and split for parallel processing
[ ! -f $pairs ] && ../../src/speech_dtw/utils/create_pair_file.py \
    $utterance_ids $pairs
...
if [ ! -d $distances_split_dir ]; then
    ...
    dist_cmd="../../src/speech_dtw/utils/calculate_dtw_costs.py \
